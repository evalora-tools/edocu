...
export async function POST(request: Request) {
  console.log('1. Iniciando creación de profesor')
  ...
  try {
    ...
    // Crear usuario profesor usando Admin API
    ...
    // Crear/actualizar el perfil del profesor usando Admin API
    ...
    if (profileError) {
      ...
      throw new Error('Error al actualizar el perfil del profesor')
    }

    // Actualizar los cursos asignados al profesor
    if (cursos_ids && cursos_ids.length > 0) {
      const { error: cursosError } = await supabaseAdmin
        .from('cursos')
        .update({ profesor_id: user.id })
        .in('id', cursos_ids)

      if (cursosError) {
        console.error('Error asignando cursos al profesor:', cursosError)
        throw new Error('Error al asignar cursos al profesor')
      }
    }

    return NextResponse.json({
      success: true,
      message: `Profesor ${nombre} creado exitosamente. Se ha enviado un correo de verificación.`
    })
